...
void CreateInfoHeader(unsigned char *info_header, const int width, const int height) {
    ...
    for (int i = 15; i < INFO_HEADER_SIZE; ++i) {  // NOLINT
        info_header[i] = 0;
    }
}
...
void SaveFile(const char *path, Image &image) {
    ...
    const int count_paddings = ((4 - (image.GetWidth() * 3) % 4) % 4);
    const int file_size = FILE_HEADER_SIZE + INFO_HEADER_SIZE + image.GetWidth() * image.GetHeight() * 3 +
                          count_paddings * image.GetHeight();
    unsigned char file_header[FILE_HEADER_SIZE];
    unsigned char info_header[INFO_HEADER_SIZE];
    CreateFileHeader(file_header, file_size);
    CreateInfoHeader(info_header, image.GetWidth(), image.GetHeight());
